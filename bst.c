...
int BSTIteratorHasNext(struct BSTreeIterator* iter) {
    while(iter->current != NULL){
        pushLinkedList(iter->s, iter->current);
        iter->current = iter->current->left;
    }
    
    return !isEmptyLinkedList(iter->s);
/* TODO: Complete this implementation */
}
...
int BSTIteratorNext(struct BSTreeIterator* iter) {
    assert(iter);
    struct node* top = (struct node*) topLinkedList(iter->s);
    popLinkedList(iter->s);
